...
def is_ai_zone_trade(trade: dict | None) -> bool:
    return bool(trade and (trade.get("execution_style") == "market" or trade.get("entry_model") == "OB_NOT_MITIGATED_ZONE_ENTRY"))
...
def build_ai_zone_market_trade(
    ...
    """
    0.5% AI zone autonomy entry.

    Used when a valid active OB/zone exists but price moves away without
    mitigating the OB. The old behavior was to skip; this creates a reduced-risk
    market entry, with strict anti-chase filters.
    """
    ...
    trade = {
        ...
        "ai_zone_reason": "Valid active zone confirmed; OB was not mitigated; reduced-risk market entry allowed.",
    }
...
def build_order_request(trade: dict, magic: int):
    if trade.get("execution_style") == "market":
        return {
...
def send_pending_order(summary: dict, trade: dict, magic: int, spread_points: float):
    ...
    success_status = "FILLED_OPEN" if trade.get("execution_style") == "market" else "ORDER_PLACED"
    success_reason = (
        "AI zone market order sent successfully."
        if trade.get("execution_style") == "market"
        else "Pending order placed successfully."
    )
    ...
    save_visual_journal_snapshot(
        "market_entry_sent" if trade.get("execution_style") == "market" else "order_sent",
        summary=summary,
        trade=trade,
        request=request,
        result=result,
